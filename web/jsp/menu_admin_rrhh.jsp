<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="controlTurnos.util.AppConfig"%>
<%@page import="controlTurnos.modelo.Empleado"%>
<%
    Empleado emp = (Empleado) session.getAttribute("empleado");
    if (emp == null || !emp.getNombreRol().equals("AdminRRHH")) {
        response.sendRedirect(request.getContextPath() + "/LoginServlet"); return;
    }
%>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Panel AdminRRHH</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/bootstrap.min.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/style.css">
</head>
<body>
<nav class="navbar navbar-custom navbar-expand-lg px-4 py-2">
    <span class="navbar-brand text-white fw-bold">Control de Turnos <span class="badge bg-secondary ms-1" style="font-size:0.65rem;vertical-align:middle;"><%=AppConfig.VERSION%></span></span>
    <div class="ms-auto d-flex align-items-center gap-3">
        <span class="text-white-50 small">
            Bienvenido, <span class="text-white fw-semibold"><%= emp.getNombreCompleto() %></span>
            &nbsp;|&nbsp; AdminRRHH
        </span>
        <a href="${pageContext.request.contextPath}/LogoutServlet" class="btn btn-outline-light btn-sm">Cerrar Sesión</a>
    </div>
</nav>
<div class="container mt-4">
    <h5 class="page-title">Panel de Administrador RRHH</h5>
    <div class="row g-4">

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/EmpleadoServlet?accion=listar" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Mantenimiento de Usuarios</h6>
                    <p class="text-muted small mb-0">Agregar, consultar y gestionar empleados</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/EmpleadoServlet?accion=roles" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Gestión de Roles</h6>
                    <p class="text-muted small mb-0">Asignar y cambiar roles a los empleados</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/EmpleadoServlet?accion=areas" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Gestión de Áreas</h6>
                    <p class="text-muted small mb-0">Editar descripción de las áreas</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/AsignacionTurnoServlet?accion=listar" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Asignación de Turnos</h6>
                    <p class="text-muted small mb-0">Ver todas las asignaciones del sistema</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/SolicitudServlet?accion=menu" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Solicitudes</h6>
                    <p class="text-muted small mb-0">Aprobar o rechazar cambios de turno y licencias</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/ReasignacionServlet?accion=listar" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Reasignación de Empleados</h6>
                    <p class="text-muted small mb-0">Cambiar área, turno y AdminArea directamente</p>
                </div>
            </a>
        </div>

        <div class="col-md-3">
            <a href="${pageContext.request.contextPath}/MarcajeServlet" class="card menu-card shadow-sm h-100 d-block p-3">
                <div class="card-body">
                    <h6 class="fw-bold mb-2">Marcaje General</h6>
                    <p class="text-muted small mb-0">Ver el marcaje de todos los empleados hoy</p>
                </div>
            </a>
        </div>

    </div>
</div>
<script src="${pageContext.request.contextPath}/js/bootstrap.bundle.min.js"></script>
</body>
</html>
